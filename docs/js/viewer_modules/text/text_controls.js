...
function createSettingsPanel() {
    ...
    panel.innerHTML = `
        ...
        <!-- 1페이지 상단 여백 -->
        <div class="setting-group" id="paddingTopGroup" style="margin-bottom: 24px;">
            <label style="display: block; font-size: 13px; color: var(--text-tertiary, #888); margin-bottom: 10px;">
        ...
        <!-- 1페이지 하단 여백 -->
        <div class="setting-group" id="paddingBottomGroup" style="margin-bottom: 24px;">
            <label style="display: block; font-size: 13px; color: var(--text-tertiary, #888); margin-bottom: 10px;">
        ...
        <!-- 2페이지 하단 여백 -->
        <div class="setting-group" id="padding2pageBottomGroup" style="margin-bottom: 24px; display: none;">
            <label style="display: block; font-size: 13px; color: var(--text-tertiary, #888); margin-bottom: 10px;">
                2Page Bottom Padding: <span id="padding2pageBottomValue">${saved2pagePaddingBottom}px</span>
            </label>
            <input type="range" id="padding2pageBottomSlider" min="0" max="100" step="5" value="${saved2pagePaddingBottom}" style="width: 100%; cursor: pointer;">
            <p style="font-size: 11px; color: var(--text-tertiary, #666); margin-top: 6px;">페이지 번호 위 글자 잘리면 늘리세요</p>
        </div>
...
function setupControlEvents() {
    ...
    if (fontMinus) fontMinus.onclick = () => changeFontSize(-2);
    if (fontPlus) fontPlus.onclick = () => changeFontSize(2);
    ...
    if (resetBtn) resetBtn.onclick = resetCurrentBookmark;
...
function updatePaddingVisibility() {
    ...
    const paddingTopGroup = document.getElementById('paddingTopGroup');
    const paddingBottomGroup = document.getElementById('paddingBottomGroup');
    const padding2pageBottomGroup = document.getElementById('padding2pageBottomGroup');
    
    if (paddingTopGroup) paddingTopGroup.style.display = is2Page ? 'none' : 'block';
    if (paddingBottomGroup) paddingBottomGroup.style.display = is2Page ? 'none' : 'block';
    if (padding2pageBottomGroup) padding2pageBottomGroup.style.display = is2Page ? 'block' : 'none';
}
...
function updateReadModeUI() {
    ...
    if (scrollBtn) scrollBtn.classList.toggle('active', currentMode === 'scroll');
    if (clickBtn) clickBtn.classList.toggle('active', currentMode === 'click');
}
...
function updateFontUI() {
    ...
    if (lineValue) lineValue.innerText = lineHeight.toFixed(1);
}
...
function updatePaddingUI() {
    ...
    const padding2pageBottom = localStorage.getItem('text_2page_padding_bottom') || '20';
    ...
    const bottom2pSlider = document.getElementById('padding2pageBottomSlider');
    const bottom2pValue = document.getElementById('padding2pageBottomValue');
    ...
    if (bottom2pSlider) bottom2pSlider.value = padding2pageBottom;
    if (bottom2pValue) bottom2pValue.innerText = padding2pageBottom + 'px';
}
...
function changeFontSize(delta) {
    ...
    applyTypography();
    const fontValue = document.getElementById('fontSizeValue');
    if (fontValue) fontValue.innerText = newSize + 'px';
}
...
export function closeSettings() {
    ...
    if (panel) {
        ...
        if (TextViewerState.ui) TextViewerState.ui.settingsOpen = false;
...
style.textContent = `
    ...
    #padding2pageBottomSlider::-webkit-slider-thumb {
        -webkit-appearance: none;
        appearance: none;
        width: 18px;
        height: 18px;
    ...
    #padding2pageBottomSlider::-moz-range-thumb {
        width: 18px;
        height: 18px;
        background: var(--accent, #4a9eff);
        border-radius: 50%;
